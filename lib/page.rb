...
class Page
  ...
  def self.analysis_output_string(pages)
    res = self.all_link_chain_strings(pages)
    res += self.most_common_chain_endings_string(pages)
  end

  def self.all_link_chain_strings(pages)
  ...
  def self.most_common_chain_endings_string(pages)
    res = ""
    chain_ends = {}
    pages.each do |page|
